...
fn offer(oid: &str, price: f64, segments: Vec<Segment>) -> Offer {
    let n = segments.len();
    let duration =
        segments.iter().map(|s| s.duration_min).sum::<i32>() + if n > 1 { 40 } else { 0 };
    ...
    Offer {
        ...
        note: Some(
            "Mock fixture. Complete priced itinerary — not a reconstructed segment sum.".into(),
        ),
        ..Offer::default()
    }
}
    ...
    pub async fn shop(&self, req: &ShopRequest) -> anyhow::Result<Vec<Offer>> {
        ...
        if req.return_date.is_some() {
            if origin == "JFK" && matches!(dest.as_str(), "ORD" | "CHI" | "MDW") {
                return Ok(vec![jfk_ord_roundtrip(
                    date,
                    req.return_date.as_deref().unwrap(),
                )]);
            }
            return Ok(vec![]);
        }
